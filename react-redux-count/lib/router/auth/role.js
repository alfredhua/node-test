...
router.post('/update-role',login_required,async(ctx,next)=>{
  const {id,name,comment,authIdList}=ctx.request.body;
  ctx.body=await postJson(auth+'/update-role',{id,name,comment,authIdList});
});
...
router.post('/create-role',login_required,async(ctx,next)=>{
  const {name,comment,authIdList}=ctx.request.body;
  ctx.body=await postJson(auth+'/create-role',{name,comment,authIdList});
});
