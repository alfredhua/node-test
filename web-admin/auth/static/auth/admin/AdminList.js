...
import { Card,Breadcrumb, message,Modal } from 'antd'
...
export default class AdminList extends React.Component{
    ...
    async load_data(page_num,page_size){
      const {code,msg,data} = await post("/auth/list-admin",{page_num,page_size});
      if(code === 'SUCCESS' ){
    ...
    async reset_password(){
      const {id} =this.state;
      const {code,msg,data} = await post("/auth/reset-password",{id});
      if(code === 'SUCCESS' ){
