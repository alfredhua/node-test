...
import { Card,Breadcrumb,Dropdown,Menu,Button,Icon,message } from 'antd';
import { Link } from 'react-router-dom'
import {time_formate} from 'common/static/util/validate';
...
export default class ProduceList extends React.Component{
    ...
    async del(id){
      const {data,code,msg}= await post('/website/del-news',{id});
      if(code === 'SUCCESS'){
    ...
    async publish(id,publish){
      const {data,code,msg}= await post('/website/update-news-publish',{id,publish});
      if(code === 'SUCCESS'){
