...
import DefaultTable from '@common/table/DefaultTable';
...
export default class NoticeTypeList extends React.Component{
    ...
    async load_data(page_num, page_size){
      const {code,data,msg}= await post('/dictionary/list-notice-type-by-page',{page_num, page_size});
      if ('SUCCESS' === code) {
    ...
    async updateActive(){
      ...
      const result= await post('/dictionary/update-notice-type-status',{id,status});
    ...
    async save(){
        ...
        let result;
        if(id===null){
          result= await post('/dictionary/create-notice-type',{name,type});
        }else{
          result= await post('/dictionary/update-notice-type',{id,name,type});
        }
        if ('SUCCESS' === result.code) {
            message.success('保存成功!');
            this.load_data(this.state.data.page_num, this.state.data.page_size);
      ...
      get_menu(data){
        return(
          <Menu>
              ...
              {data.status===0?
              <AuthButton auth_list={this.props.auth_list}  auth_path='/dictionary/notice-type/active' onClick={() => { this.setState({id:data.id,status:1,act_visible:true})}} auth_type='menu-item'>激活</AuthButton>:
              <AuthButton auth_list={this.props.auth_list}  auth_path='/dictionary/notice-type/close' onClick={() => { this.setState({id:data.id,status:0,act_visible:true})}} auth_type='menu-item'>关闭</AuthButton>}
              <AuthButton auth_list={this.props.auth_list}  auth_path='/dictionary/notice-type/watch-notice' onClick={() => { this.props.history.push(`/website/list-notice/${data.type}`) }} auth_type='menu-item'>查看文章内容</AuthButton>
              {/* <AuthButton auth_list={this.props.auth_list}  auth_path='/dictionary/notice-type/del' onClick={() => { this.setState({id:data.id,act_visible:true})}}  auth_type='menu-item'>删除</AuthButton> */}
          </Menu>
        );
      }
    render(){
