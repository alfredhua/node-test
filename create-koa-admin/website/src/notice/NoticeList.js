...
export  default class NoticeList extends React.Component{
 ...
 constructor(props){
  ...
  async load_data(page_num,page_size,type){
    ...
    const notice_type_result=await post('/dictionary/list-all-notice-type',{});
  ...
  render(){
    const columns = [{
      ...
      render: (data) => (
        <span>
            {data.publish===0?
            <AuthButton auth_list={this.props.auth_list} auth_path='/website/notice/publish' auth_type='tag' checked={true} onClick={() => {this.setState({id:data.id,publish_modal:true,type:'PUBLISH'}) }} >发布</AuthButton>:null}
            ...
            <AuthButton auth_list={this.props.auth_list} auth_path='/website/notice/del'  auth_type='tag' checked={true} onClick={() => {this.setState({id:data.id,publish_modal:true,type:'DELETE'})}} >删除</AuthButton>
        </span>
      )
    }
    ];
