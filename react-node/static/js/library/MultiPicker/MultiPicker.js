...
(function (wid, dcm) {
	...
	function MultiPicker(config) {
		...
		this.idxArr    = [];
		this.jsonArr   = [];
		this.liHeight  = wid.lib ? Number(doc.getElementsByTagName('HTML')[0].style.fontSize.replace('px', '')) * 1 :34;
		this.maxHeight = [];
		this.distance  = [];
		this.start     = {
			Y: 0,
	...
	MultiPicker.prototype = {
		...
		initDomFuc: function () {
			...
			html += '<div class="multi-picker-bg" id="multi-picker-bg-' + _this.container + '">'
						+ '<div class="multi-picker-wrapper">'
							+ '<div class="multi-picker-btn close" id="multi-picker-btn-cancel"></div>'
							+ '<div class="multi-picker-btn confirm" id="multi-picker-btn-save-' + _this.container + '">确定</div>'
							+ '<div  class="multi-picker-container" id="multi-picker-container-' + _this.container + '">'
								+ '<div class="multi-picker-btn-box"></div>'
