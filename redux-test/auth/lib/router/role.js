...
router.post('/create-role',login_required,async(ctx,next)=>{
  ...
  ctx.body=await postJson(auth+'/create-role',{name,comment,auth_list:JSON.stringify(auth_list)});
});
...
router.post('/list-role',login_required,async(ctx,next)=>{
  ...
  ctx.body=await postJson(auth+'/list-role',{page_num,page_size});
});
...
router.post('/update-role',login_required,async(ctx,next)=>{
  ...
  ctx.body=await postJson(auth+'/update-role',{id,name,comment,auth_list:JSON.stringify(auth_list)});
});
